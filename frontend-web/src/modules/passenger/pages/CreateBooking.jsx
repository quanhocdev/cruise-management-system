// src/modules/passenger/pages/CreateBooking.jsx
...
export default function CreateBooking() {
  ...
  if (loading)
    return (
      <div className="catalog-state full-page">
        Đang kiểm tra phòng đã chọn...
      </div>
    );
  if (error && !room) {
    return (
      <div className="catalog-state full-page error-state">
        <p>{error}</p>
        <Link to={`/passenger/tours/${tourId || ""}`}>Chọn lại phòng</Link>
      </div>
    );
  }
    ...
    return (
      <main className="passenger-page booking-result-page">
        <section className="booking-result-card">
          ...
          <p>
            Booking <strong>#{booking.id}</strong> đang chờ thanh toán.
          </p>
          ...
          <small>
            Giao dịch sử dụng môi trường VNPay Sandbox, không trừ tiền thật.
          </small>
        </section>
      </main>
    );
  ...
  return (
    <main className="passenger-page create-booking-page">
      <header className="booking-form-header">
        <Link
          to={`/passenger/tours/${tourId}`}
          className="back-link static-back"
        >
          <ArrowLeft size={18} /> Chọn lại phòng
        </Link>
        <div>
          <span className="eyebrow">HOÀN TẤT THÔNG TIN</span>
          <h1>Đặt chỗ hành trình</h1>
        </div>
      </header>
      ...
      <form className="booking-form-layout" onSubmit={submitBooking}>
          ...
          <section className="detail-panel form-section">
            <div className="form-section-title">
              <div>
                ...
                <p>Tối đa {room.remainingCapacity} hành khách cho phòng này.</p>
              </div>
              <button
                type="button"
              ...
              >
                <UserPlus size={17} /> Thêm hành khách
              </button>
            </div>
        ...
        <aside className="booking-summary booking-form-summary">
          ...
          <div className="summary-row">
            <Ship size={18} /> {tour.cruiseName}
          </div>
          <div className="summary-row">
            <CalendarDays size={18} /> {tour.startDate}
          </div>
          ...
          <div className="selected-room-summary">
            ...
            <span>
              ...
              <small>
                Phòng {room.roomCode} · Tầng {room.deckNumber}
              </small>
            </span>
          </div>
          ...
          <button
            ...
            disabled={submitting}
          >
            {submitting ? "Đang tạo booking..." : "Xác nhận giữ chỗ"}
          </button>
          <small>
            Giá chính thức được Booking Service tính lại từ dữ liệu phòng.
          </small>
        </aside>
      </form>
    </main>
