...
// try {
// TourScheduleContext result =
// client.get().uri("/internal/voyages/{id}/booking-context", voyageId)
// .header("X-Internal-Api-Key",
// apiKey).retrieve().body(TourScheduleContext.class);
// if (result == null) throw new BookingException(HttpStatus.BAD_GATEWAY, "Tour
// service returned an empty response");
// return result;
// } catch (BookingException ex) { throw ex; }
// catch (Exception ex) { throw new BookingException(HttpStatus.BAD_GATEWAY,
// "Cannot validate voyage: " + ex.getMessage()); }
// }

// public List<TourRoomContext> getRooms(UUID voyageId) {
// try {
// TourRoomContext[] result = client.get().uri("/internal/voyages/{id}/rooms",
// voyageId)
// .header("X-Internal-Api-Key",
// apiKey).retrieve().body(TourRoomContext[].class);
// return result == null ? List.of() : Arrays.asList(result);
// } catch (Exception ex) {
// throw new BookingException(HttpStatus.BAD_GATEWAY, "Cannot load voyage rooms:
// " + ex.getMessage());
// }
// }
// }