...
// public BookingTripDetails get(@PathVariable Long id, @AuthenticationPrincipal
// Jwt jwt) {
// Long userId;
// try { Object claim = jwt.getClaim("userId"); userId =
...
// return new BookingTripDetails(trip.voyageId(), trip.tourName(),
// trip.cruiseName(),
// trip.startDate(), trip.endDate(), trip.rooms() == null ? java.util.List.of()
// : trip.rooms().stream()
// .filter(r -> assignedRooms.contains(r.roomId())).toList(), trip.itinerary(),
// trip.activities(), trip.catalog());
// }
// }
