...
// public PosIdentityService(PosService authentication, PosTerminalRepository
// terminals,
// PassengerVoyageRepository passengers, PosPassengerCredentialRepository
// credentials) {
...
// .map(item -> new TerminalSummary(item.getId(), item.getCode(),
// item.getName(),
...
// .map(item -> new PassengerSummary(item.getId(),
// item.getPassenger().getFullName(),
...
// return
// credentials.findAllByPassengerVoyage_VoyageIdOrderByCreatedAtDesc(voyageId).stream()
// .map(item -> new CredentialSummary(item.getId(),
// item.getPassengerVoyage().getId(),
...
// .orElseThrow(() -> new BookingException(HttpStatus.NOT_FOUND, "POS terminal
// not found"));
// if (!passengers.existsByVoyageId(voyageId))
// throw new BookingException(HttpStatus.NOT_FOUND, "Voyage has no registered
// booking passengers");
...
// .orElseThrow(() -> new BookingException(HttpStatus.NOT_FOUND, "Passenger
// voyage not found"));
// if (passenger.getBooking().getStatus() != BookingStatus.CONFIRMED ||
// passenger.getPassengerStatus() != PassengerStatus.REGISTERED)
// throw new BookingException(HttpStatus.CONFLICT, "Only confirmed passengers
...
// if (value == null) throw new BookingException(HttpStatus.BAD_REQUEST, "NFC
// UID must contain 4, 7 or 10 bytes of hex");
// } else throw new BookingException(HttpStatus.BAD_REQUEST, "Unsupported scan
// type");
// String fingerprint = fingerprint(request.scanType(), value);
// if (credentials.existsByFingerprint(fingerprint))
// throw new BookingException(HttpStatus.CONFLICT, "Credential already
// registered; use another card");
// var credential = new PosPassengerCredential();
// credential.setFingerprint(fingerprint);
// credential.setScanType(request.scanType());
// credential.setPassengerVoyage(passenger);
// credential.setCreatedAt(Instant.now());
// credentials.save(credential);
// return new CredentialIssued(credential.getId(), passenger.getId(),
// request.scanType(), value);
// }

// @Transactional
// public void revoke(Long id) {
// var credential = credentials.findById(id)
// .orElseThrow(() -> new BookingException(HttpStatus.NOT_FOUND, "Credential not
// found"));
...
// if (terminal.getAssignedVoyageId() == null) return
// Identity.rejected("TERMINAL_NOT_ASSIGNED");
// String value;
// if ("NFC".equals(request.scanType())) value =
// normalizeNfc(request.scannedValue());
// else if ("QR".equals(request.scanType()) &&
// request.scannedValue().trim().matches("POS:[A-Za-z0-9_-]{43}"))
// value = request.scannedValue().trim();
// else value = null;
// if (value == null) return Identity.rejected("INVALID_CODE");
// var credential =
// credentials.findByFingerprint(fingerprint(request.scanType(),
// value)).orElse(null);
...
// if (!terminal.getAssignedVoyageId().equals(link.getVoyageId())) return
// Identity.rejected("WRONG_VOYAGE");
// if (!link.getVoyageId().equals(link.getBooking().getVoyageId())) return
// Identity.rejected("INVALID_BOOKING");
...
// return checkInResult("ALREADY_CHECKED_IN", link,
// link.getCheckedInTerminalCode());
...
// private CheckInResult checkInResult(String status, PassengerVoyage link,
// String terminalCode) {
// return new CheckInResult(status, null, link.getId(),
// link.getPassenger().getFullName(),
// link.getBooking().getBookingCode(), link.getVoyageId(), link.getCabinId(),
...
// String uid = input.trim().replace(":", "").replace(" ",
...
// } catch (java.security.NoSuchAlgorithmException ex) { throw new
