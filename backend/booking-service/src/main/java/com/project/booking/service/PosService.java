...
// @Transactional
// public PosTerminalRegistrationResponse register(RegisterPosTerminalRequest
// request) {
// String code = request.code().trim().toUpperCase();
// if (terminalRepository.existsByCodeIgnoreCase(code))
// throw new BookingException(HttpStatus.CONFLICT, "POS terminal code already
// exists");
...
// return new PosTerminalRegistrationResponse(saved.getId(), saved.getCode(),
// saved.getName(), secret);
// }

// @Transactional
// public PosSyncResponse sync(String terminalCode, String posKey,
// PosSyncRequest request) {
...
// ? "SHA256:" + PosIdentityService.fingerprint(request.scanType(),
...
// if (terminalCode == null || terminalCode.isBlank() || posKey == null ||
// posKey.isBlank())
// throw new BookingException(HttpStatus.UNAUTHORIZED, "POS credentials are
// required");
// PosTerminal terminal =
// terminalRepository.findByCodeIgnoreCase(terminalCode.trim())
// .orElseThrow(() -> new BookingException(HttpStatus.UNAUTHORIZED, "Invalid POS
// credentials"));
// if (!terminal.isActive() || !passwordEncoder.matches(posKey,
// terminal.getSecretHash()))
// throw new BookingException(HttpStatus.UNAUTHORIZED, "Invalid POS
// credentials");
// return terminal;
// }

// private PosSyncResponse response(PosTransaction transaction, boolean
// duplicate) {
// return new PosSyncResponse(transaction.getId(), transaction.getLocalId(),
// "SYNCED",
