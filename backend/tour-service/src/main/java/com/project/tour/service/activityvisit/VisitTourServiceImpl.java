...
import com.project.tour.mapper.activityvisit.TourVisitSyncMapper;
import com.project.tour.mapper.activityvisit.VisitTourMapper;
import com.project.tour.model.activityvisit.ScheduleOfActivityVisit;
import com.project.tour.model.activityvisit.ScheduleStopOfActivityVisit;
...
import com.project.tour.repository.activityvisit.TourOfAcitvityVisitRepository;
...
public class VisitTourServiceImpl implements VisitTourService {
    ...
    public VisitTourServiceImpl(
        ...
        this.tourOfAcitvityVisitRepository =
                tourOfAcitvityVisitRepository;
    }
    ...
    public VisitTourResponse create(
        ...
        // 1. Tìm bản ghi VisitTour đã được sinh sẵn từ sự kiện Kafka
        // dựa vào scheduleStopId
        ...
        // 2. Cập nhật các thông tin cấu hình từ form Frontend
        ...
        // Chuyển trạng thái sang đã cấu hình
    ...
    // =====================================================
    // KAFKA - SYNC TOUR MASTER DATA
    // =====================================================

    @Override
    @Transactional
    public void syncTourMasterData(
            TourMasterSyncEvent event) {

        // 1. Tạo hoặc cập nhật Tour chính
        TourOfAcitvityVisit tour =
                tourOfAcitvityVisitRepository
                        .findById(event.tourId())
                        .orElseGet(() -> {

                            TourOfAcitvityVisit newTour =
                                    new TourOfAcitvityVisit();
        ...
        // Clear danh sách cũ nếu đồng bộ lại
        tour.getSchedules().clear();

        // 2. Map và lưu danh sách Schedules
        if (event.schedules() != null) {

            List<ScheduleOfActivityVisit> scheduleEntities =
                    new java.util.ArrayList<>();

            for (TourMasterSyncEvent.ScheduleDetail sDetail
                    : event.schedules()) {

                ScheduleOfActivityVisit schedule =
                        new ScheduleOfActivityVisit();
                ...
                // 3. Map và lưu danh sách Stops
                if (sDetail.stops() != null) {

                    List<ScheduleStopOfActivityVisit> stopEntities =
                            new java.util.ArrayList<>();

                    for (TourMasterSyncEvent.ScheduleStopDetail stopDetail
                            : sDetail.stops()) {

                        ScheduleStopOfActivityVisit stop =
                                new ScheduleStopOfActivityVisit();
        ...
        // Lưu toàn bộ phân cấp nhờ CascadeType.ALL
        tourOfAcitvityVisitRepository.save(tour);
    }

    // =====================================================
    // GET MASTER TOUR BY ID
    // =====================================================

    @Override
    @Transactional(readOnly = true)
    public TourVisitSyncResponse getMasterTourById(
            UUID tourId) {
        ...
        TourOfAcitvityVisit tour =
                tourOfAcitvityVisitRepository
                        .findById(tourId)
                        .orElseThrow(() -> new AppException(
                                "Master tour not found for visit sync",
                                HttpStatus.NOT_FOUND));
    ...
    public List<TourVisitSyncResponse> getAllMasterTours() {
        ...
        return tourOfAcitvityVisitRepository
                .findAll()
                .stream()
