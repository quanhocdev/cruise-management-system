...
// public
// InternalFeedbackTargetController(ActivityCruiseTourAssignmentRepository
// onboardRepository,
// VisitTourRepository shoreRepository, ProductTourAssignmentRepository
// productRepository,
...
// public FeedbackTargetContext context(@PathVariable UUID tourId, @PathVariable
// String targetType,
...
// default -> throw new AppException("Unsupported feedback target type: " +
// targetType, HttpStatus.BAD_REQUEST);
// };
// }
...
// return result(tourId, "ONBOARD_ACTIVITY", targetId, item.getStatus() ==
// ActivityCruiseTourStatus.COMPLETED);
// }
...
// ProductTour item = productRepository.findById(targetId).orElseThrow(() ->
// notFound(targetId));
// verifyTour(tourId, item.getTour().getId());
// return result(tourId, "PRODUCT", targetId, item.getStatus() ==
// ProductTourStatus.COMPLETED);
// }

// private FeedbackTargetContext service(UUID tourId, UUID targetId) {
// ServiceTour item = serviceRepository.findById(targetId).orElseThrow(() ->
// notFound(targetId));
// verifyTour(tourId, item.getTour().getId());
// return result(tourId, "SERVICE", targetId, item.getStatus() ==
// ServiceTourStatus.COMPLETED);
// }

// private FeedbackTargetContext result(UUID tourId, String type, UUID targetId,
// boolean completed) {
// return new FeedbackTargetContext(tourId, type, targetId, completed);
// }

// private void verifyTour(UUID requested, UUID actual) {
// if (!requested.equals(actual)) throw new AppException("Feedback target does
// not belong to this tour", HttpStatus.CONFLICT);
// }

// private AppException notFound(UUID id) {
// return new AppException("Feedback target not found: " + id,
// HttpStatus.NOT_FOUND);
// }

// private void authorize(String suppliedApiKey) {
// byte[] supplied = suppliedApiKey == null ? new byte[0] :
