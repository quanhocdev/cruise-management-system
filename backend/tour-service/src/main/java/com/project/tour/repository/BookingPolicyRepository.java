...
public interface BookingPolicyRepository extends JpaRepository<BookingPolicy, UUID> {
    Optional<BookingPolicy> findByIdAndPolicy_Id(UUID id, UUID policyId);
    List<BookingPolicy> findAllByPolicy_IdOrderByDaysBeforeDepartureDesc(UUID policyId);
    List<BookingPolicy> findAllByPolicy_IdAndStatusOrderByDaysBeforeDepartureDesc(
        UUID policyId, PolicyStatus status
    );
    boolean existsByPolicy_IdAndDaysBeforeDeparture(UUID policyId, Integer days);
    boolean existsByPolicy_IdAndDaysBeforeDepartureAndIdNot(
        UUID policyId, Integer days, UUID id
    );
}
