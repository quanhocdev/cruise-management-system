...
// public InternalScheduleController(ScheduleRepository repository,
// @Value("${internal.api-key}") String key) {
// this.repository = repository; this.expectedKey =
...
// .orElseThrow(() -> new ResourceNotFoundException("Schedule not found with id:
// " + id));
// return new ScheduleBookingContext(schedule.getId(), schedule.getCapacity(),
// schedule.getStartDate(), schedule.getStatus());
// }
// private void authorize(String key) {
// byte[] actual = key == null ? new byte[0] :
// key.getBytes(StandardCharsets.UTF_8);
// if (!MessageDigest.isEqual(expectedKey, actual))
// throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid internal
// API key");
// }
// }
