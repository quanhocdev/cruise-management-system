...
class ScheduleServiceTests {
    ...
    @Mock CruiseRepository cruiseRepository;
    ScheduleService service;
    UUID packageId; UUID cruiseId; TourPackage tourPackage; Cruise cruise;

    @BeforeEach void setUp() {
        service = new ScheduleService(repository, packageRepository, cruiseRepository);
        packageId = UUID.randomUUID(); cruiseId = UUID.randomUUID();
        tourPackage = new TourPackage(); tourPackage.setId(packageId); tourPackage.setName("Ha Long");
        tourPackage.setNumberOfDays(3); tourPackage.setStatus(TourPackageStatus.ACTIVE);
        cruise = new Cruise(); cruise.setId(cruiseId); cruise.setName("Ocean Star");
        cruise.setMaxPassengers(200); cruise.setStatus(CruiseStatus.ACTIVE);
    }

    @Test void createLinksReferencesAndNormalizesCode() {
    ...
    @Test void createRejectsCruiseDateConflict() {
        when(packageRepository.findById(packageId)).thenReturn(Optional.of(tourPackage));
        when(cruiseRepository.findById(cruiseId)).thenReturn(Optional.of(cruise));
    ...
    @Test void createRejectsDurationDifferentFromPackage() {
        when(packageRepository.findById(packageId)).thenReturn(Optional.of(tourPackage));
        when(cruiseRepository.findById(cruiseId)).thenReturn(Optional.of(cruise));
        LocalDate start = LocalDate.of(2026, 9, 1);
