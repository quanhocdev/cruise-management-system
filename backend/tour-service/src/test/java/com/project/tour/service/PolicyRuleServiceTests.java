...
class PolicyRuleServiceTests {
    @Mock CancelPolicyRepository cancelRepository;
    @Mock BookingPolicyRepository bookingRepository;
    @Mock PolicyService policyService;

    @Test
    void cancellationRuleRejectsDuplicateThreshold() {
        UUID policyId = UUID.randomUUID();
        Policy policy = new Policy(); policy.setType(PolicyType.CANCEL);
    ...
    void bookingRuleRejectsDuplicateThreshold() {
        UUID policyId = UUID.randomUUID();
        Policy policy = new Policy(); policy.setType(PolicyType.BOOKING);
