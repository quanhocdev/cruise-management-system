...
class InternalScheduleControllerSecurityTests {
    ...
    @Test void missingInternalKeyIsRejected() throws Exception {
        mockMvc.perform(get("/internal/schedules/{id}/booking-context", ID)).andExpect(status().isUnauthorized());
    }
    @Test void correctInternalKeyReturnsContext() throws Exception {
        Schedule schedule = new Schedule(); schedule.setId(ID); schedule.setCapacity(100);
        schedule.setStartDate(LocalDate.now().plusDays(10)); schedule.setStatus(ScheduleStatus.OPEN);
