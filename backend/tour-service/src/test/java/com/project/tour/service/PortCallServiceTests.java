...
class PortCallServiceTests {
    @Mock PortCallRepository repository;
    @Mock ItineraryDayRepository dayRepository;
    @Mock PortRepository portRepository;

    @Test void createRejectsDepartureBeforeArrival() {
        UUID scheduleId = UUID.randomUUID(); UUID dayId = UUID.randomUUID();
        UUID portId = UUID.randomUUID();
        ItineraryDay day = new ItineraryDay(); day.setItineraryDate(LocalDate.of(2026, 9, 1));
        Port port = new Port(); port.setId(portId); port.setStatus(PortStatus.ACTIVE);
