...
public class TestNotificationServiceImpl implements TestNotificationService {
    ...
    public TestNotificationServiceImpl(NotificationRepository repository, EmailDeliveryService emailDeliveryService) {
    ...
    public NotificationResponse create(CreateNotificationRequest request) {
        ...
        n.setRecipientEmail(blankToNull(request.recipientEmail()));
        n.setType(request.type());
        n.setTitle(request.title().trim());
        n.setMessage(request.message().trim());
        n.setReferenceType(blankToNull(request.referenceType()));
        ...
        emailDeliveryService.deliver(saved);
    ...
    public UnreadCountResponse unreadCount(Long userId) {
        return new UnreadCountResponse(repository.countByRecipientUserIdAndReadAtIsNull(userId));
    }
    ...
    public void markAllRead(Long userId) {
        ...
        List<Notification> items = repository.findAllByRecipientUserIdAndReadAtIsNull(userId);
        items.forEach(n -> n.setReadAt(now));
        repository.saveAll(items);
    }

    private Notification owned(Long id, Long userId) {
        Notification n = repository.findById(id)
                .orElseThrow(() -> new NotificationException(HttpStatus.NOT_FOUND, "Notification not found: " + id));
        if (!Objects.equals(n.getRecipientUserId(), userId))
            throw new NotificationException(HttpStatus.FORBIDDEN, "You cannot access this notification");
        return n;
    }

    private NotificationResponse toResponse(Notification n) {
        return new NotificationResponse(n.getId(), n.getRecipientUserId(),
                n.getRecipientEmail(), n.getType(), n.getTitle(), n.getMessage(), n.getReferenceType(),
                n.getReferenceId(),
